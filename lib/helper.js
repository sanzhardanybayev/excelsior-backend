...
helpers.to = promise =>
  promise.then(data => [null, data]).catch(err => [err.message]);
 ...
 * @return {*}
 */
helpers.genToken = UserId => jwt.sign({ id: UserId }, config.JWT_KEY);

helpers.verifyToken = async token => {
  try {
    const { id } = jwt.verify(token, "needs");
    const [err, user] = await to(User.findById(id));
    return err || !user ? false : [true, user];
  } catch (err) {
    return [false, {}];
  }
};

helpers.getToken = req => {
  return req
